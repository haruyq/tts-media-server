...
MeloTTS ZH と Kokoro-82M は別GitHubプロジェクトへ分離されており、
メディアサーバーのPython環境には入りません。`plugins/` はHTTPクライアント
のみで、PyTorchとモデルは独立したDockerコンテナ内で動作します。
...
| --- | --- | --- |
| MeloTTS ZH | `http://127.0.0.1:50100` | [hayabusa255/melotts_zh_plugin](https://github.com/hayabusa255/melotts_zh_plugin) |
| Kokoro-82M | `http://127.0.0.1:50101` | [hayabusa255/kokoro-82m-plugin](https://github.com/hayabusa255/kokoro-82m-plugin) |

`compose.yaml`のbuild contextは、それぞれ検証済みの外部リポジトリcommitへ
固定しています。更新時は各エンジン側を先にテスト・pushし、そのcommit SHAを
Composeへ反映してください。エンジンのDockerfile、重い依存、モデル実装、
単体テストはTTS Media Serverリポジトリには含めません。
