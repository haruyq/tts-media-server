...
最終的な`HybridReadingResult.tts_text`では、単語間を含む空白文字を除去して読みを連結します。たとえば`Minecraft Discord`は`まいんくらふとでぃすこーど`になります。位置確認用の`prepared_text`と`segments`には空白を含む原文座標を維持します。
