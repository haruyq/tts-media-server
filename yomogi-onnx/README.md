...
最終的な`HybridReadingResult.tts_text`では、Yomogiのtoken／segment境界に空白を追加せず読みを連結します。たとえば`脱北者が`は`だっぽくしゃが`です。一方、入力に明示された空白は保持します。`Minecraft Discord`は`まいんくらふと でぃすこーど`になり、位置確認用の`prepared_text`と`segments`にも同じ入力座標を維持します。
