...
Yomogi v1.4を、PyTorchを本番環境へ持ち込まずONNX Runtimeで実行するための変換・推論実装です。Hugging Face Space `prj-beatrice/yomogi-v1` のコミット `3135d1274edf66099fbced229b0048b08e98dd70`（短縮形`3135d12`）に固定しています。
...
- 全1,022文の`dict_id`列、surface/read/pron列、最終read/pron、正規化文字列: 100%完全一致
- pytest: 9 passed（うち一致テストが上記1,022文×方式A/B）
- PyTorchなしのPython 3.11新規環境: 同期推論・非同期推論とも合格

詳細は`reports/equivalence_fp32.json`、`reports/benchmark.json`、`dist/export_report.json`を参照してください。FP32で不一致はありません。INT8では60件の不一致があり、`reports/equivalence_int8_failures.jsonl`に入力、両出力、候補ID、候補logit差を保存しています。
...
Python 3.11〜3.13を対象にしています。本番の直接依存は`numpy`と`onnxruntime`だけです。
...
Windowsでは`.venv\Scripts\pip.exe`と`.venv\Scripts\python.exe`を使用してください。JSONには`read_katakana`、`pron_katakana`、`pron_hiragana`、TTS既定値の`tts_text`、各tokenの`dict_id`、未知区間が含まれます。
...
空文字は安全に空結果を返します。最大長の既定値は500文字で、`max_length=`から変更できます。超過は黙って切らず`ValueError`です。辞書候補がなく読みから消えた部分は`unknown_spans`に位置と原文を返すため検出できます。
...
`tests/ambiguous_sentences.txt`の22文に加え、辞書表層を固定seed 3135で組み合わせた1,000文を比較します。不一致時は入力、PyTorch/ONNX結果、候補ID、候補logit差をJSONLへ出します。候補の並び、`ordered_candidates()`、貪欲デコードの進行方法は変更していません。
...
- 未知文字IDは元実装どおり0です。辞書で覆えない部分は最終read/pronには入りませんが、`unknown_spans`で必ず検出できます。
...
Yomogi v1.4のMIT表記を`LICENSE`に維持しています。辞書はpyopenjtalk-plus/NAIST-jdic/UniDic、AzooKey関連、NEologd、SudachiDict、Mozc UT辞書などの由来を持ち、それぞれの条件が適用されます。元READMEの由来表記は`THIRD_PARTY_NOTICES.md`に保持しています。再配布・配備前にリンク先の各ライセンス全文も確認してください。
